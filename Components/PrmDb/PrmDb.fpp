module Components {
    ...
    passive component PrmDb {
        ...
        @ Command to save parameters to file with name passed to constructor
        guarded command PRM_SAVE_FILE \
        opcode 0
        ...
        ) severity warning low id 0 format \
        "Parameter ID 0x{x} not found" \
        throttle 5

        @ Parameter ID updated in database
        event PrmIdUpdated(
            Id: U32 @< The parameter ID
        ) severity activity high id 1 format \
        "Parameter ID 0x{x} updated"

        @ Parameter database is full
        event PrmDbFull(
            Id: U32 @< The parameter ID
        ) severity fatal id 2 format \
        "Parameter DB full when adding ID 0x{x} "

        @ Parameter ID added to database
        event PrmIdAdded(
            Id: U32 @< The parameter ID
        ) severity activity high id 3 format \
        "Parameter ID 0x{x} added"
        ...
        ) severity warning high id 4 format \
        "Parameter write failed in stage {} with record {} and error {}"

        @ Save of parameter file completed
        event PrmFileSaveComplete(
            records: U32 @< The number of records saved
        ) severity activity high id 5 format \
        "Parameter file save completed. Wrote {} records."
        ...
        event PrmFileReadError(
            ...
            $record: I32 @< The record that had the failure
            error: I32 @< The error code
        ) severity warning high id 6 format \
        "Parameter file read failed in stage {} with record {} and error {}"

        @ Load of parameter file completed
        event PrmFileLoadComplete(
            records: U32 @< The number of records loaded
        ) severity activity high id 7 format \
        "Parameter file load completed. Read {} records."
